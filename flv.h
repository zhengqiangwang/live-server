...
public:
    ...
    int32_t m_timestampDelta;
    // 3bytes.
    // Three-byte field that represents the size of the payload in bytes.
    // It is set in big-endian format.
    int32_t m_payloadLength;
    // 1byte.
...
public:
    ...
    void InitializeAmf0Script(int size, int stream);
    // Create a audio header, set the size, timestamp and stream_id.
    void InitializeAudio(int size, uint32_t time, int stream);
    // Create a video header, set the size, timestamp and stream_id.
    void InitializeVideo(int size, uint32_t time, int stream);
};
...
public:
    ...
    virtual void CreatePayload(int size);
public:
    ...
    virtual error Create(MessageHeader* pheader, char* body, int size);
};
...
public:
    ...
    virtual error Create(CommonMessage* msg);
    // Create shared ptr message,
    // from the header and payload.
    // @remark user should never free the payload.
    // @param pheader, the header to copy to the message. NULL to ignore.
    virtual error Create(MessageHeader* pheader, char* payload, int size);
...
public:
    ...
    virtual int ChunkHeader(char* cache, int nb_cache, bool c0);
public:
    // copy current shared ptr message, use ref-count.
    // @remark, assert object is created.
    virtual SharedPtrMessage* Copy();
    // Only copy the buffer, without header fields.
    virtual SharedPtrMessage* Copy2();
};
...
private:
    IWriter* m_writer;
private:
    char m_tagHeader[FLV_TAG_HEADER_SIZE];
public:
    FlvTransmuxer();
    virtual ~FlvTransmuxer();
public:
    ...
    virtual error Initialize(IWriter* fw);
public:
    // Write flv header.
    // Write following:
    ...
    virtual error WriteHeader(char flv_header[9]);
    // Write flv metadata.
    ...
    virtual error WriteMetadata(char type, char* data, int size);
...
private:
    virtual void CacheMetadata(char type, char* data, int size, char* cache);
    virtual void CacheAudio(int64_t timestamp, char* data, int size, char* cache);
    virtual void CacheVideo(int64_t timestamp, char* data, int size, char* cache);
    virtual void CachePts(int size, char* cache);
    virtual error WriteTag(char* header, int header_size, char* tag, int tag_size);
};
...
class FlvVodStreamDecoder
{
private:
    FileReader* m_reader;
public:
    FlvVodStreamDecoder();
    virtual ~FlvVodStreamDecoder();
public:
    // Initialize the underlayer file stream
    // @remark user can initialize multiple times to decode multiple flv files.
    // @remark user must free the @param fr, flv decoder never close/free it.
...
public:
    ...
    // @remark assert header not NULL.
    virtual error ReadHeaderExt(char header[13]);
    ...
    virtual error ReadSequenceHeaderSummary(int64_t* pstart, int* psize);
public:
    // For start offset, seed to this position and response flv stream.
    virtual error Seek2(int64_t offset);
};
