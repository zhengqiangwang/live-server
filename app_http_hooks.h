#ifndef APP_HTTP_HOOKS_H
#define APP_HTTP_HOOKS_H
...
public:
    ...
    //         ignore if empty.
    static error OnConnect(std::string url, Request* req);
    // The on_close hook, when client disconnect to srs, where client is valid by on_connect.
    // @param url the api server url, to process the event.
    //         ignore if empty.
    static void OnClose(std::string url, Request* req, int64_t send_bytes, int64_t recv_bytes);
    // The on_publish hook, when client(encoder) start to publish stream
    // @param url the api server url, to valid the client.
    //         ignore if empty.
    static error OnPublish(std::string url, Request* req);
    // The on_unpublish hook, when client(encoder) stop publish stream.
    // @param url the api server url, to process the event.
    //         ignore if empty.
    static void OnUnpublish(std::string url, Request* req);
    // The on_play hook, when client start to play stream.
    // @param url the api server url, to valid the client.
    //         ignore if empty.
    static error OnPlay(std::string url, Request* req);
    // The on_stop hook, when client stop to play the stream.
    // @param url the api server url, to process the event.
    //         ignore if empty.
    static void OnStop(std::string url, Request* req);
    ...
    static error OnDvr(ContextId cid, std::string url, Request* req, std::string file);
    ...
    // @param cid the source connection cid, for the on_dvr is async call.
    static error OnHls(ContextId cid, std::string url, Request* req, std::string file, std::string ts_url,
        std::string m3u8, std::string m3u8_url, int sn, utime_t duration);
    // When hls reap segment, callback.
    // @param url the api server url, to process the event.
    //         ignore if empty.
    ...
    // @param cid the source connection cid, for the on_dvr is async call.
    static error OnHlsNotify(ContextId cid, std::string url, Request* req, std::string ts_url, int nb_notify);
    // Discover co-workers for origin cluster.
    static error DiscoverCoWorkers(std::string url, std::string& host, int& port);
    // The on_forward_backend hook, when publish stream start to forward
    // @param url the api server url, to valid the client.
    //         ignore if empty.
    static error OnForwardBackend(std::string url, Request* req, std::vector<std::string>& rtmp_urls);
private:
    static error DoPost(HttpClient* hc, std::string url, std::string req, int& code, std::string& res);
};

#endif // APP_HTTP_HOOKS_H
