...
#define ERRNO_GEN(n, v, m, s) n = v,
enum ErrorCode{
#ifndef _WIN32
    ERROR_SUCCESS = 0,
#endif
    ERRNO_MAP_SYSTEM(ERRNO_GEN)
    ERRNO_MAP_RTMP(ERRNO_GEN)
    ERRNO_MAP_APP(ERRNO_GEN)
    ERRNO_MAP_HTTP(ERRNO_GEN)
    ERRNO_MAP_USER(ERRNO_GEN)
};
#undef ERRNO_GEN

//charge the error whether system control error
...
//the error carry code, message, callstack and instant variables,
//which easy to locate problem by log
class ComplexError
{
private:
    int m_code;
    ComplexError* m_wrapped;
    std::string m_msg;
...
private:
    ComplexError();
public:
    virtual ~ComplexError();
private:
    virtual std::string Description();
    virtual std::string Summary();
public:
    static ComplexError* Create(const char* func, const char* file, int line, int code, const char* fmt, ...);
    static ComplexError* Wrap(const char* func, const char* file, int line, ComplexError* v, const char* fmt, ...);
    static ComplexError* Success();
    static ComplexError* Copy(ComplexError* from);
    static std::string Description(ComplexError* err);
    static std::string Summary(ComplexError* err);
    static int ErrorCode(ComplexError* err);
    static std::string ErrorCodeStr(ComplexError* err);
    static std::string ErrorCodeLongstr(ComplexError* err);
public:
    static void Assert(bool expression);
};

//error help,use these functions to new or wrap an error
#define SUCCESS 0
#define ERRORNEW(ret, fmt, ...)    ComplexError::Create(__FUNCTION__, __FILE__, __LINE__, ret, fmt, ##__VA_ARGS__)
#define ERRORWRAP(err, fmt, ...) ComplexError::Wrap(__FUNCTION__, __FILE__, __LINE__, err, fmt, ##__VA_ARGS__)
#define ERRORCOPY(err) ComplexError::Copy(err)
#define ERRORDESC(err) ComplexError::Description(err)
#define ERRORSUMMARY(err) ComplexError::Summary(err)
#define ERRORCODE(err) ComplexError::ErrorCode(err)
#define ERRORCODESTR(err) ComplexError::ErrorCodeStr(err)
#define ERRORCODELONGSTR(err) ComplexError::ErrorCodeLongstr(err)
#define ERRORRESET(err) Freep(err); err = SUCCESS

#ifndef Assert
#define Assert(expression) ComplexError::Assert(expression)
#endif
#endif // ERROR_H
